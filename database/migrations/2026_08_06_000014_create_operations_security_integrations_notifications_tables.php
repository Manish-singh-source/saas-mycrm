...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('remote_login_sessions', function (Blueprint $table) {
            $table->id();
            $table->uuid('uuid')->unique();
            $table->foreignId('tenant_id')->constrained('tenants')->restrictOnDelete();
            $table->foreignId('platform_user_id')->constrained('platform_users')->restrictOnDelete();
            $table->foreignId('impersonated_user_id')->nullable()->constrained('users')->restrictOnDelete();
            $table->string('reason');
            $table->string('status', 50)->default('active');
            $table->dateTime('started_at')->nullable();
    ...
    {
        foreach (['tenant_api_tokens', 'remote_login_sessions', 'knowledge_base_articles', 'knowledge_base_categories', 'platform_ticket_comments', 'platform_tickets', 'tenant_restore_requests', 'tenant_backup_runs', 'backup_runs', 'notification_templates', 'communication_logs', 'notifications', 'integration_rate_limits', 'integration_field_mappings', 'integration_sync_jobs', 'integration_webhook_logs', 'integration_webhooks', 'integration_credentials', 'tenant_integrations', 'integration_providers', 'monitoring_alerts', 'system_incidents', 'security_events', 'scheduler_logs', 'queue_job_logs', 'api_request_logs', 'tenant_usage_snapshots', 'monitoring_service_logs', 'monitoring_services'] as $table) {
