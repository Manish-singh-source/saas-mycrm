...
return new class extends Migration
{
    public function up(): void
    {
        ...
        Schema::create('teams', function (Blueprint $table) {
            ...
            $table->foreignId('team_type_id')->nullable()->constrained('tenant_lookups')->nullOnDelete();
            $table->string('name', 150);
            $table->string('code', 80);
            $table->text('description')->nullable();
            $table->foreignId('lead_user_id')->nullable()->constrained('users')->nullOnDelete();
            $table->foreignId('assistant_lead_user_id')->nullable()->constrained('users')->nullOnDelete();
            $table->string('email', 150)->nullable();
            $table->string('phone', 20)->nullable();
            $table->string('color', 30)->nullable();
            $table->string('icon', 80)->nullable();
            $table->string('visibility', 50)->default('tenant');
        ...
        Schema::create('staff', function (Blueprint $table) {
            ...
            $table->index(['tenant_id', 'department_id', 'employment_status']);
            $table->index(['tenant_id', 'primary_team_id']);
        });
        Schema::create('team_members', function (Blueprint $table) {
            ...
            $table->foreignId('updated_by')->nullable()->constrained('users')->nullOnDelete();
            $table->timestamps();
            $table->unique(['tenant_id', 'team_id', 'user_id', 'effective_from']);
        });
        Schema::create('team_permissions', function (Blueprint $table) {
            ...
            $table->unique(['tenant_id', 'team_id', 'permission_id']);
        });
        Schema::create('team_settings', function (Blueprint $table) {
            ...
            $table->timestamps();
            $table->unique(['tenant_id', 'team_id', 'group', 'key']);
        });
        Schema::create('team_assignments', function (Blueprint $table) {
            ...
            $table->string('status', 50)->default('active');
            $table->index(['tenant_id', 'assignable_type', 'assignable_id'], 'team_assignable_idx');
        });
        ...
        Schema::create('staff_salary_structures', function (Blueprint $table) {
            ...
            $table->date('effective_from');
            $table->date('effective_to')->nullable();
            $table->decimal('annual_ctc', 18, 2)->default(0);
            $table->decimal('monthly_gross', 18, 2)->default(0);
        ...
        Schema::create('staff_documents', function (Blueprint $table) {
            ...
            $table->foreignId('tenant_id')->constrained('tenants')->cascadeOnDelete();
            $table->foreignId('staff_id')->constrained('staff')->cascadeOnDelete();
            $table->foreignId('document_type_id')->nullable()->constrained('tenant_lookups')->nullOnDelete();
        ...
        Schema::create('leave_requests', function (Blueprint $table) {
            ...
            $table->dateTime('approved_at')->nullable();
            $table->timestamps();
            $table->index(['tenant_id', 'staff_id', 'start_date']);
        });
        Schema::create('leave_balances', function (Blueprint $table) {
            ...
            foreach (['opening_balance', 'accrued', 'used', 'remaining'] as $column) {
                $table->decimal($column, 8, 2)->default(0);
            }
            $table->unique(['tenant_id', 'staff_id', 'leave_type_id', 'year']);
        });
    }
    public function down(): void
    {
        foreach (['leave_balances', 'leave_requests', 'leave_types', 'attendance_requests', 'attendance_records', 'staff_shift_assignments', 'shifts', 'staff_training', 'staff_appraisals', 'staff_certifications', 'staff_assets', 'staff_emergency_contacts', 'staff_documents', 'staff_salary_structures', 'staff_bank_accounts', 'staff_employment_history', 'team_assignments', 'team_settings', 'team_permissions', 'team_members', 'staff', 'team_roles', 'teams', 'designations', 'departments'] as $table) {
