...
curl -X PATCH "{{BASE_URL}}/api/platform/v1/support/tickets/{ticket_uuid}" -H "Authorization: Bearer {{PLATFORM_TOKEN}}" -H "Content-Type: application/json" -H "Accept: application/json" -d '{"tenant_uuid":"tenant_uuid","subject":"Invoice not generated","description":"July invoice missing","priority":"urgent","category":"billing","source":"platform","assigned_to_uuid":"platform_user_uuid","status":"pending"}'
...
curl -X POST "{{BASE_URL}}/api/platform/v1/support/tickets/{ticket_uuid}/attachments" -H "Authorization: Bearer {{PLATFORM_TOKEN}}" -H "Accept: application/json" -F "file=@/path/to/screenshot.png" -F "visibility=private" -F "purpose=support_ticket"
