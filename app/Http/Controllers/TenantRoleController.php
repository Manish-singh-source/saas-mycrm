...
class TenantRoleController extends BaseApiController
{
    ...
    {
        $baseQuery = Role::where('tenant_id', $this->tenant->id())->where('guard_name', 'tenant');
    ...
    public function bulkDestroy(Request $request)
    {
        ...
        $roles = Role::where('tenant_id', $this->tenant->id())->where('guard_name', 'tenant')->whereIn('uuid', $data['role_uuids'])->get();
        $deleted = 0;
        $skipped = 0;
    ...
    public function replaceUsers(Request $request, $role_uuid)
    {
        $role = $this->findRole($role_uuid);
    ...
    {
        return Role::where('tenant_id', $this->tenant->id())->where('guard_name', 'tenant')->where('uuid', $uuid)->firstOrFail();
    }

    private function permissionIds(array $uuids)
    {
        return Permission::where('guard_name', 'tenant')->whereIn('uuid', $uuids)->pluck('id')->all();
    }
