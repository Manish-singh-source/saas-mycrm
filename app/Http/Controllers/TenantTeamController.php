...
class TenantTeamController extends BaseApiController
{
    ...
    public function show(string $team_uuid): JsonResponse
    {
        ...
        $payload = (array) $team;
        $payload['department'] = $team->department_id ? DB::table('departments')->where('id', $team->department_id)->first(['uuid', 'name']) : null;
        $payload['office'] = $team->office_id ? DB::table('tenant_offices')->where('id', $team->office_id)->first(['uuid', 'office_name']) : null;
        $payload['lead_user'] = $team->lead_user_id ? DB::table('users')->where('id', $team->lead_user_id)->first(['uuid', 'display_name', 'email']) : null;
        $payload['assistant_lead_user'] = $team->assistant_lead_user_id ? DB::table('users')->where('id', $team->assistant_lead_user_id)->first(['uuid', 'display_name', 'email']) : null;
        return $this->success(['team' => $payload + [
