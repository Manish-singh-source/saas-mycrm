...
class PlatformMonitoringController extends BasePlatformController
{
    ...
    public function queueJobs(Request $request) { $q = DB::table('queue_job_logs'); $this->applyStatusFilter($q, $this->statusFilter($request)); $p = $q->latest('id')->paginate((int) $request->integer('per_page', 25)); return $this->list($p->items(), $p); }
    ...
    public function alerts(Request $request) { $q = DB::table('monitoring_alerts'); $this->applyStatusFilter($q, $this->statusFilter($request)); $p = $q->latest('id')->paginate((int) $request->integer('per_page', 25)); return $this->list($p->items(), $p); }
    ...
    public function incidents(Request $request) { $q = DB::table('system_incidents'); $this->applyStatusFilter($q, $this->statusFilter($request)); $p = $q->latest('id')->paginate((int) $request->integer('per_page', 25)); return $this->list($p->items(), $p); }
