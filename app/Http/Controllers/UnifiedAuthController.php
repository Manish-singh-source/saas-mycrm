...
final class UnifiedAuthController extends Controller
{
    ...
    public function login(Request $request): mixed
    {
        ...
        if (! $discovery || $discovery['email'] !== $input['email']) {
            return ApiResponse::error('Discovery token expired.', 401, null, 'DISCOVERY_TOKEN_EXPIRED');
        }
        $selected = $discovery['accounts'][$input['account_ref']] ?? null;
        if (! $selected) {
            return ApiResponse::error('Invalid account reference.', 422, null, 'INVALID_ACCOUNT_REF');
        }
        [$surface, $id] = $selected;
        $model = $surface === 'platform' ? PlatformUser::find($id) : User::find($id);
        if (! $model || $model->email !== $input['email'] || $model->status !== 'active' || ! Hash::check($input['password'], $model->password)) {
            return ApiResponse::error('Invalid credentials.', 401, null, 'INVALID_CREDENTIALS');
        }
    ...
    public function verifyTwoFactor(Request $request): mixed
    {
        ...
        if (! $challenge) {
            return ApiResponse::error('Two-factor challenge expired.', 401, null, 'TWO_FACTOR_CHALLENGE_EXPIRED');
        }
        $model = $challenge['model_type']::find($challenge['model_id']);
        if (! $model || ! $this->validTotp($model->two_factor_secret, $input['code'])) {
            return ApiResponse::error('Invalid two-factor code.', 422, null, 'INVALID_2FA_CODE');
        }
    ...
    private function issueToken(Request $request, object $model, string $deviceName): mixed
    {
        ...
        $tenant = $isPlatform ? null : $model->load('tenant')->tenant;
        $abilities = $isPlatform
            ? $model->roles()->where('platform_roles.status', 'active')->with('permissions')->get()->flatMap(fn ($role) => $role->permissions->where('status', 'active')->pluck('name'))->merge($model->permissions()->where('platform_permissions.status', 'active')->pluck('name'))->unique()->values()->all()
            : array_values(array_unique(array_merge(
                $this->tenantPermissions($model),
    ...
    private function account(object $user, string $type, mixed $tenantId): array
    {
        ...
        return ['account_ref' => $ref, 'id' => $user->getKey(), 'account_type' => $type, 'auth_guard' => $user instanceof PlatformUser ? 'platform' : 'tenant', 'surface' => $user instanceof PlatformUser ? 'platform' : 'tenant', 'display_label' => $user->display_name, 'email' => $user->email, 'organization' => $tenant?->organization_name, 'tenant_id' => $tenant?->uuid ?? $tenantId, 'tenant' => $tenant?->only(['uuid', 'slug', 'status', 'organization_name', 'display_name']), 'roles' => [], 'status' => $user->status, 'last_login_at' => $user->last_login_at];
    }
